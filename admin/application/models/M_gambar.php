...
class M_gambar extends CI_Model{	
	...
	function tambah_link($kat,$id,$author,$link){
		...
		$this->db->insert('tb_gambar', $data_upload);
	}

	function tambah_gambar($kat,$id,$author,$files){		
	...
	function tambah_gambar_single($kat,$id,$author,$files){
	...
	function getAllGambar(){
		$res = array();
		$ar = $this->db->query("SELECT * FROM tb_gambar;")->result_array();
		$i = 1;
	...
	function getGambar($id){
		$res = array();
		$ar = $this->db->query("SELECT * FROM tb_gambar WHERE kat_id = '$id';")->result_array();
		$i = 1;
		foreach ($ar as $key => $value) {
	...
	function hapusGambar($id){
		$datagambar = $this->db->query("SELECT direktori FROM tb_gambar WHERE id = '$id'")->result_array();
		// echo base_url($datagambar[0]['direktori']);
		if(file_exists('./'.$datagambar[0]['direktori'])) {
			     unlink('./'.$datagambar[0]['direktori']);
		}
		$this->db->where('id', $id);
		$this->db->delete('tb_gambar');
	}
